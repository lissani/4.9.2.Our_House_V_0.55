...
void Perspective_Camera::define_camera(int win_width, int win_height, float win_aspect_ratio) {
	...
	case CAMERA_CC_0: {
		...
		cam_proj.projection_type = CAMERA_PROJECTION_PERSPECTIVE;
		cam_proj.params.pers.fovy = 65.0f * TO_RADIAN; // ī�޶� �þ߰�
		cam_proj.params.pers.aspect = win_aspect_ratio;
		cam_proj.params.pers.n = 1.0f;
	...
	case CAMERA_CC_DYNAMIC: {
		...
		glm::vec3 cctv_position = glm::vec3(100.0f, 75.0f, 45.0f);  // �ǹ� �߾� õ��
		glm::vec3 cctv_target = glm::vec3(80.0f, 60.0f, 10.0f);     // �Ž� �ٴ� ��
...
void Orthographic_Camera::define_camera(int win_width, int win_height, float win_aspect_ratio) {
	switch (camera_id) {
	case CAMERA_SIDE_FRONT: {// ���鵵
		flag_valid = true;
		flag_move = false;

		glm::vec3 eye_pos = glm::vec3(115.0f, -300.0f, 25.0f); // ���鿡��
		glm::vec3 target = glm::vec3(115.0f, 50.0f, 25.0f); // �ǹ� �߽� �ٶ󺸱�
