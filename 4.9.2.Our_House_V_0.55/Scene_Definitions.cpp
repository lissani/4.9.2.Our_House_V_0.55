...
void Axis_Object::draw_camera_axis(Shader_Simple* shader_simple,
	...
	RotationMatrix[0] = glm::vec4(glm::normalize(uaxis), 0.0f);    // X축
	RotationMatrix[1] = glm::vec4(glm::normalize(vaxis), 0.0f);    // Y축  
	RotationMatrix[2] = glm::vec4(glm::normalize(-naxis), 0.0f);   // Z축
...
void Scene::draw_camera_frames() {
	...
	static bool debug_printed = false;  // 정적 변수로 한 번만 출력
	...
	debug_printed = true;  // 첫 번째 호출 후 플래그 설정
}
